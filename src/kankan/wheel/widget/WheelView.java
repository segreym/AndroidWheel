...
public class WheelView extends View implements WheelScroller.ScrollingListener {
     ...
     */
    private void initData(Context context, AttributeSet attrs, final int defStyle) {
        this.scroller = new WheelScroller(context, this);
        this.dataObserver = new WheelViewAdapterDataSetObserver(this);
        if (attrs != null) {
            final TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.WheelView, defStyle, 0);
            try {
                topShadow = a.getDrawable(R.styleable.WheelView_topShadow);
                bottomShadow = a.getDrawable(R.styleable.WheelView_bottomShadow);
                centerDrawable = a.getDrawable(R.styleable.WheelView_centerDrawable);
            } catch (Exception e) {
                Log.e(this.getClass().getName(), "Failed to read view attributes", e);
            } finally {
                a.recycle();
            }
        }
    ...
    private void initResourcesIfNecessary() {
        if (centerDrawable == null) {
            centerDrawable = getDefaultCenterDrawable();
        }
    ...
    private Drawable getDefaultCenterDrawable() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            //noinspection deprecation
            return getContext().getResources().getDrawable(R.drawable.wheel_val);
        } else {
            return getContext().getDrawable(R.drawable.wheel_val);
        }
    }
    
    
    private static class WheelViewAdapterDataSetObserver extends DataSetObserver {
        private final WeakReference<WheelView> wheelRef;

        WheelViewAdapterDataSetObserver(final WheelView wheel) {
            this.wheelRef = new WeakReference<WheelView>(wheel);
        }

        @Override
        public void onChanged() {
            invalidateWheelView(false);
        }

        @Override
        public void onInvalidated() {
            invalidateWheelView(true);
        }
        
        private void invalidateWheelView(final boolean clearCaches) {
            final WheelView wheelView = wheelRef.get();
            if (wheelView != null) {
                wheelView.invalidateWheel(clearCaches);
            }
        }
